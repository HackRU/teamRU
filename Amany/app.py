...
def register(email: str, token: str):
    ...
    if resp_parsed["statusCode"] == 200:
        if (database.save_skills(email, request.form['skills'])) == 0:
            return jsonify({"statusCode": 400, "body": "You already have a profile"})
        if (database.save_inf(email, [request.form['slack'], request.form['github_handle'], request.form['no_of_hackathons']])) ==0:
