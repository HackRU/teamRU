...
def save_skills(email: str, inf: str):
...
def save_inf(email: str, inf: []):
    cnx = open_database_connection()
    mycursor = cnx.cursor()
    query = "INSERT INTO innodb.participants (email, slack, github_handle, no_of_hackathons, has_team) VALUES ('" + email + "', '" + inf[0].lower() + "', '" + inf[1].lower() + "', " + str(inf[2]) + ", 0" + ")"
    try:
        mycursor.execute(query)
    except ms.IntegrityError as err:
        cnx.close()
...
def get_possible_candidates():
    ...
    for language in skills_list:
        query = "SELECT p.email from innodb.participants p ,innodb.skills l where p.email = l.email and p.no_of_hackathons >= '" + str(hackathons) + "'  and l.skill = '" + language.strip().lower() + "' and p.has_team = 0"
        mycursor.execute(query)
...
def has_team(email):
    cnx = open_database_connection()
    mycursor = cnx.cursor()
    query = "UPDATE  innodb.participants SET has_team = 1 WHERE email = '" + email + "'"
    mycursor.execute(query)
    cnx.commit()
    cnx.close()